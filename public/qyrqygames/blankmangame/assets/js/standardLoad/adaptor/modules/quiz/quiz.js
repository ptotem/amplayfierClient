...
config.quiz = {
    ...
    locations: [
        {name: "statement-area", states: [
            {name: "default", representation: "<img src='assets/img/statement-back.png'/>"}
        ]},
        {name: "options", states: [
            {name: "default", representation: ""}
        ]},
...
Question.getAnswer = function(question, all) {
    
    var tmp = question.options;
    var correctAnswers = []
    for(var i=0; i<tmp.length; i++){
        if(tmp[i].correct=="true") correctAnswers.push(tmp[i].name);
    }
    if (all){
...
Question.showQuizPanel = function (obj, question) {
    $('#statement-area').html('<div id="question-image"></div>'+question.name);
    $('#options').empty().append("<ul></ul>");
    if(question.image != null){
        $('#question-image').empty().html("<img src='assets/img/" + question.image + "' />");
    }
    for (var i in question.options) {
        $('#options ul').append('<li class="option-block" id="option-block-' + i + '"><div class="option-text">' + question.options[i].name + '</div></li>');
    }
