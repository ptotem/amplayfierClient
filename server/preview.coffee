...
@previewIP = Meteor.settings.previewIP



@getTenantHtml = (tid, secretKey, res, isMaster)->
  x = DDP.connect(remoteIp)
  x.call('requestHTMLForTenantPreview', tid, secretKey, res, (err, res)->
    console.log err
    console.log res
    if !err
#      deckHtml.remove({})
...
@getTenantJs = (tid, secretKey, res, isMaster)->
  x = DDP.connect(remoteIp)
  x.call('requestJSForTenantPreview', tid, secretKey, res, isMaster,  (err, res)->
    console.log err
    console.log res
    if !err
      p = platforms.findOne({tenantId: tid})
...
@getTenantMetaData = (tid, secretKey, res, isMaster)->
  x = DDP.connect(remoteIp)
  x.call('requestTenantMetaDataPreview', tid, secretKey, res, isMaster, (err, res)->
    console.log err
    console.log res
    if !err
      # Meteor.call('syncTenantAssets',res,tid)
      platforms.update({tenantId: tid},
        {$set: {backgroundUrl: res.backImage, platformLogo: res.logoImage, tenantIcon: res.favIc, platformLinks: res.socialLinks}})
  )

@getIntegratedGames = (tid, secretKey, res)->
  x = DDP.connect(remoteIp)
  x.call('syncTenantGamesPreview', tid, secretKey, res, (err, res)->
    console.log err
    console.log res
...
@getIntegratedGameQuestions = (tid, secretKey, res, isMaster)->
  x = DDP.connect(remoteIp)
  x.call('syncIntegratedGameQuestionsPreview', tid, secretKey, res, isMaster, (err, res)->
    console.log err
    console.log res
    if !err
      p = platforms.findOne({tenantId: tid})
...
@getSubGameQuestions = (tid, secretKey, res, isMaster)->
  x = DDP.connect(remoteIp)
  x.call('syncSubGameQuestionsPreview', tid, secretKey, res, isMaster, (err, res)->
    console.log err
    # console.log res
    if !err
      p = platforms.findOne({tenantId: tid})
...
@getCustomizationData = (tid, secretKey, res, isMaster)->
  x = DDP.connect(remoteIp)
  x.call('syncCustomizationDataPreview', tid, secretKey, res, isMaster, (err, res)->
    console.log err
    console.log res
    if !err
      p = platforms.findOne({tenantId: tid})
...
@getRequestForTenant = (tid, secretKey, res, isMaster)->
  x = DDP.connect(remoteIp)
  x.call('requestStoryWrapperForTenantPreview', tid, secretKey, res, isMaster, (err, res)->
    console.log err
    console.log res
    pname = platforms.findOne({tenantId: tid}).tenantName
    if !err
      platforms.update({tenantId: tid}, {$set: {questions:res.questions,tenantName: pname, nodes: res.nodes, storyConfig: res.sconfig,wrapperJson:res.wrapperJson,gameGuide:res.gameGuide, gameIntro:res.gameIntro}})
  )

@getAllAssetsForTenant = (tid, secretKey, res)->
  x = DDP.connect(remoteIp)
  x.call('requestAssetForTenantPreview', tid, secretKey, res, (err, res)->
    console.log err
    console.log res
    if !err
      console.log "-------------------------------------------------------------"
...
Meteor.methods
  ...
  assignUserProfilePreview: (uid, prf)->
    Meteor.users.update({_id: uid}, {$set: {profile: prf}})
    true

  createAdminOnClientPreview: (email, tid, platformName,platformPwd)->
  ...
  fetchDataFromCreatorPreview: (tid)->
    x = DDP.connect(remoteIp)
    ...
    x.call('checkCreatorConnectionPreview', tid, secretKey, (err, res)->
      console.log err
      console.log res
      if res isnt -1
  ...
  authorizeConnectionPreview: (tid, tname)->
    x = DDP.connect(remoteIp)
  ...
  makePlatformReadyForSynPreview: (tid, secretKey)->
    if !platforms.findOne({tenantId: tid})?
      platforms.update({tenantId: tid}, {$set: {platformSync: false}})
      return true
    else
      return false
  deletePlatformPreview:(tid,tenantName,secretKey)->
  ...
  createPlatformPreview: (tid, tname, secretKey)->
    if !platforms.findOne({tenantId: tid})?
...
#      platforms.update({_id:"AqFLFgDvD5hMBQ8Zh"},{$set:{}})
  ...
  buildPlatformsForClientPreview: (tid, subPlatforms, tname, secretKey)->
    if !platforms.findOne({tenantId: tid})?
      # TODO:Archive Platforms before wiping current platform
      # archivePlatforms.insert({platformData:platforms.findOne({tenantId:tid})})
      # platforms.remove({tenantId:tid})
#      platforms.update({_id:"AqFLFgDvD5hMBQ8Zh"},{$set:{}})
  ...
  isReadyForCommunicationPreview: (secretKey)->
  ...
  storeHtmlPreview: (tid, did, tname, dname, htmlString)->
...
#      platforms.remove({})
  ...
  storeNodesPreview: (sConfig, nodes, tid, tname)->
    console.log "Syncing "
  ...
  syncTenantAssetsPreview: (fl, tid)->
    p = platforms.findOne({tenantId: tid})
    filelist = " "
    exec = Npm.require('child_process').exec
    pwd = process.env["PWD"]
  ...
  readWrapperImagesPreview: (f, listOfFiles)->
    console.log "----------------------------------"
  ...
  addReportPreview: (repObj) ->
    reports.insert(repObj)
  updateReportPreview: (queryString, parameters) ->
    reports.update(queryString, {$set: parameters})

  updateGameReportPreview: (queryString, parameters) ->
  ...
  bulkInsertUsersPreview: Meteor.bindEnvironment((fileid, pid) ->
  ...
  bulkInsertAssessmentQuestionsPreview: Meteor.bindEnvironment((fileid, pid,statementId) ->
    platformName = platforms.findOne(pid).tenantName
    file = excelFiles.findOne(fileid)
    Fiber = Npm.require('fibers');
    Future = Npm.require('fibers/future');
  ...
  updateUserPreview: (uid, p)->
    Meteor.users.update({_id: uid}, {
      $set:
        personal_profile: p
    })
    return true
  updateUserRolePreview: (uid, newRole)->
    Meteor.users.update({_id: uid}, {
      $set:
  ...
  removeUserPreview: (uid)->
    Meteor.users.remove({_id: uid})
    return true

  getPlatformTypePreview: (platformId)->
  ...
  addIndividualUserPreview: (p, pid)->
...
#    ul = 1
  ...
  forgotMyPasswordPreview:(email,captchaData)->
  ...
  updateFlunkCountPreview:(uid)->
    Meteor.users.update({_id:uid},{$inc:{flunkCount:1}})
  setUserScoreFromGamePreview:(uid,scr)->
    Meteor.users.update({_id:uid},{$inc:{points:scr}})
  setUserCurrencyFromGamePreview:(uid,scr)->
    Meteor.users.update({_id:uid},{$inc:{currency:scr}})
  disbaleFeaturePreview:(pid,uid,feature,fs)->
